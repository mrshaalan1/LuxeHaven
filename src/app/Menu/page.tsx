...
function Menu() {
 ...
 return (
   <div className="Menu bg-sky">
     ...
     {dishs &&
        dishs.dish
          .filter((dish) => dish.RestaurantItemType === filter || filter === '')
          .filter((dish) => dish.RestaurantItemName.toLowerCase().includes(query.toLowerCase()))
          .sort((a, b) => {
            if(sort === 'asc') return a.RestaurantItemPrice - b.RestaurantItemPrice;
            else if(sort === 'desc') return b.RestaurantItemPrice - a.RestaurantItemPrice;
            else return 0;
          })
          .map(({
            RestaurantItemId,
